...
describe('longestSubcommandTermLength', () => {
  it('when no commands then returns zero', () => {
    const program = new commander.Command();
    const helper = new commander.Help();
    expect(helper.longestSubcommandTermLength(program, helper)).toEqual(0);
  });

  it('when command and no help then returns length of term', () => {
  ...
  it('when command with arg and no help then returns length of term', () => {
  ...
  it('when multiple commands then returns longest length', () => {
  ...
  it('when just help command then returns length of help term', () => {
    const program = new commander.Command();
    program
      .addHelpCommand(true);
    const helper = new commander.Help();
