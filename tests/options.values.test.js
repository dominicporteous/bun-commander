...
for(const value of ['str', '80', '-', '-5', '--flag']) {
describe(
  `option with required value specified as ${value}`, () => {
    ...
    it('when short flag followed by value then value is as specified', () => {
      const program = createPortProgram();
      program.parse(['node', 'test', '-p', value]);
      expect(program.opts().port).toBe(value);
    });

    it('when short flag concatenated with value then value is as specified', () => {
      const program = createPortProgram();
      program.parse(['node', 'test', `-p${value}`]);
      expect(program.opts().port).toBe(value);
    });

    it('when long flag followed by value then value is as specified', () => {
      const program = createPortProgram();
      program.parse(['node', 'test', '--port', value]);
      expect(program.opts().port).toBe(value);
    });

    it('when long flag = value then value is as specified', () => {
      const program = createPortProgram();
      program.parse(['node', 'test', `--port=${value}`]);
      expect(program.opts().port).toBe(value);
    });
...
describe('option with optional value', () => {
  ...
  it('when long flag followed empty string then value is empty string', () => {
    const program = createPortProgram();
    program
