...
describe('longestArgumentTermLength', () => {
  it('when no arguments then returns zero', () => {
    const program = new commander.Command();
    const helper = new commander.Help();
    expect(helper.longestArgumentTermLength(program, helper)).toEqual(0);
  });

  it('when has argument description then returns argument length', () => {
    const program = new commander.Command();
  ...
  it('when has multiple argument descriptions then returns longest', () => {
    const program = new commander.Command();
